...
# 사후 주석 : ctrl + /
'''
다중 주석 : 작은따옴표 세 개 - 솔직히 말해서 얘는 다중 str 입력할 때 쓰는 기호
    에 해당합니다. 그런데 얘를 변수에 대입하지 않았기 때문에 그냥 다중 주석
    용으로 쓰는 편
'''
print(1)            # 숫자 자료형
print('1')          # 문자열 자료형
...
그래서 방금 30 번 라인에서 여러분은 str()이라는 함수를 봤습니다.
str() : 다른 데이터를 문자열 자료형으로 바꿔주는 함수
int() : 문자열/실수 자료형을 정수형으로 바꿔주는 함수
float() : 실수 자료형으로 바꿔주는 함수
근데 매번 바꿔주기 귀찮다 -> 변수 선언할 때는 자료형 명시 안하면서 print()할 때마다
매번 통제해야하냐
