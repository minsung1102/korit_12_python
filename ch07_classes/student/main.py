...
#getter의 호출     - 주의 사항 : 객체명. 속성명도 아니고 객체명.메서드명() - 소괄호열고 닫고도 아니라는 점을 주목해야 합니다
...
원래는 이거 자동 생성되기 때문에 일일이 @ 달고 _속성명인지 / 그냥 속성명인지 따질 필요가 없는데 저희는 backend를 파이썬으로 짜지 않기 때문에 현재 쓸모가 없는 상황입니다. 현재 수준으로는 python으로 백엔드를 작성할 때 필요할 때가 있고, 이렇게 getter / setter를 쓴다는 거승로 알아두시면 되겠습니다.
'''
