'''
아까 전에 일종의 setter 같은 것을 이용하여 속성에 속성값을 대입해줬습니다. 굳이 field 선언을 하지 않았는데 method 내에서 객체의 속성을 정의할 수 있따는 점이 특이했습니다. 그런데 이렇게 메서드를 경유하게 되면 기본 생성자를 통해서 객체를 생성한 다음에 속성에 값을 대입해야 하는 과정을 거쳐야 하는 것을 알 수 있습니다.

객체 생성시 기본 생성자 호출 → set_info() 메서드 호출해서 값 대입

그러니까, AllArgsConstructor에 해당하는 걸 python에서 정의할 수 있지 않을까
'''
...
Java / JS에서의 생성자 명은 클래스 명과 동일하거나 constructor를 쓰는데, python만 또 혼자서 이상한거 씁니다. __init__()입니다. 언더스코어 두 개임.
'''
class Candy2:
    # 생성자 정의
...
print()
# 객체 소멸자 호출 방법
del sample         # del 객체명입니다 소멸자는 객체 메모리를 날려야하기 때문에 객체에 종소깅 아닙니다.
print('객체 소멸 이후 작성한 코드 입니다. 프로그램 종료 전에 이미 객체가 삭제되었음을 확인할 수 있습니다.')
'''
