...
1. 클래스 변수 vs. 인스턴스 변수
    ...
    클래스 변수 접근 방식 - 인스턴스 접근(o) / 클래스 접근(x)
'''
# 클래스 변수 예시
class Korean:
    country = '한국'
...
이상을 이유로 클래스 변수를 확인하고자 할 때는 객체명.클래스변수명 보다는 클래스명.클래스변수명을 통해서 참조하는 것이 권장됩니다.
...
class Korean2:
    ...
    # 클래스 메서드의 정의 방법
    @classmethod            # @ decorator 를 달면 됩니다.
...
# 객체 생성
preson2 = Korean2()
preson2.trip('일본')      # 객체명.클래스메서드() 호출도 가능하긴 합니다. 근데 마찬가지로 권장되지 않습니다.
'''
3. 정적 메서드(static method)
    정적 메서드 또한 self를 쓰지 않음. 즉, 인스턴스 변수에 접근하여 사용하나ㅡㄴ 것이 불가능함을 의미. self.속성명을 사용하여 인스턴스 변수의 값을 참조하는데 정적 메서드는 아예 첫번째 매개변수가 고정되어 있지 않습니다. - 인스턴스변수를 참조하지 못하다는 점에서 클래스 메서드와의 공통점에 해당.
...
class Korean3:
    ...
    def slogan2(str_example):
        print(f'Imagin Yout Korea ! {str_example}')
...
class Bag:
    ...
    def __init__(self): # 여기 내부나 인스턴스메서드 내부에서 self쓰면 속성이 선언되겠네요.
        ...
        # 즉, 생성자는 인스턴스 메서드이기 때문에 인스턴스 메서드 내에서 클래스 변수를 참조할 때는 cls.클래스변수명이 아니라 클래스명.클래스변수명 으로 참조해야 한다는 점이 중요합니다.
...
# 객체 생성
bag1 = Bag()
print(f'현재 가방 재고 : {Bag.count}')    # 인스턴스메서드(생성자)를 통해서 클래스 변수의 값을 바꿨습니다. 이 값은 모든 Bag 클래스의 인스턴스들이 공유한다는 점에서 정적 변수 개념과 동일합니다.
bag2 = Bag()
bag3 = Bag()
