...
for letter in str_example:
    print(letter, end='')
'''
마이너스 인덱스 : 문자열의 뒤에서부터 부여하는 번호. 맨 마지막 데이터의 인덱스
넘버는 -1

문자열 슬라이싱(slice) : 문자열의 인덱스를 활용하여 한 문자 이상으로 구성된
단어나 문장을 추출할 때 사용하는 방법
    추출하고자 하는 단어나 문장의 시작 인덱스와 종료 인덱스를 통해 그 사이
    문자들을 추출하는 것이 가능함.
...
변수명[시작인덱스:종료인덱스:증감값]이랑 똑같아보입니다. 근데 왜 하나는 : 이고
다른 하나는 , 일까요
...
네 자리 숫자를 입력하세요 >>> 2026
맨 마지막 숫자는 6입니다.
맨 마지막 숫자는 6이며 짝수입니다.        -> 조건문 써야 합니다.
'''
number = input('네 자리 숫자를 입력하세요 >>> ')
print(type(number))         # <class 'str'>
print(f'맨 마지막 숫자는 {number[-1]}입니다.')    # str은 인덱싱이 가능
# 조건문 작성 시
if int(number[-1]) % 2 == 0:        # 조건문 시점에는 % 연산해야하니까 형변환
